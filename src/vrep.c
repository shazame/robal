...
static robal_from_vrep_t buffer_from_vrep;
static robal_to_vrep_t buffer_to_vrep;
...
static void * robal_vrep_task(void * args)
{
  ...
  robal_to_vrep_t buffer_to_vrep_sync;
  ...
  while (simxGetConnectionId(clientID)!=-1)
  {
    ...
    memcpy(&buffer_from_vrep, &buffer_from_vrep_unsync, sizeof(robal_from_vrep_t));
    memcpy(&buffer_to_vrep_sync, &buffer_to_vrep, sizeof(robal_to_vrep_t));
...
void robal_vrep_init(void)
{
  ...
  if(pthread_create(&vrep_thread, NULL, robal_vrep_task, NULL) != 0)
  {
...
void robal_vrep_position_set_x(float x)
{
  ...
  {
    printf("robal_vrep_position_set_x: Connection to VREP remote API server not yet established\n");
    exit(EXIT_FAILURE);
  }
}
...
void robal_vrep_position_set_y(float y)
{
  ...
  {
    printf("robal_vrep_position_set_y: Connection to VREP remote API server not yet established\n");
    exit(EXIT_FAILURE);
  }
}
...
void robal_vrep_position_set_angle(float angle)
{
  ...
  {
    printf("robal_vrep_position_set_angle: Connection to VREP remote API server not yet established\n");
    exit(EXIT_FAILURE);
  }
}
